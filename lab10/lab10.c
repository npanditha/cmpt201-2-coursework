...
int main() {
  ...
  for (int i = 0; i < NUM_MSG; i++) {
    sleep(1);
    // prepare message
    // this pads the destination with NULL
...
1. How is the client sending data to the server? What protocol?
   The client sends data to the server using a TCP socket. It creates an
   AF_INET, SOCK_STREAM socket and connects to 127.0.0.1 on port 8001.

2. What data is the client sending to the server?
   The client sends five messages to the server: "Hello", "Apple", "Car",
   "Green", and "Dog". Each message is copied into a 1024-byte buffer and
   written to the socket.

Understanding the Server:
1. Explain the argument that the `run_acceptor` thread is passed as an argument.
   The run_acceptor thread is passed a pointer to a struct acceptor_args.
   This struct contains a run flag used to control when the acceptor thread
   should stop, a pointer to the shared list_handle where received messages
   are stored, and a pointer to the mutex used to protect that shared list.

2. How are received messages stored?
   Received messages are stored in a linked list. Each received message is
   copied into dynamically allocated memory, placed inside a dynamically
   allocated list_node, and appended to the shared list using add_to_list().

3. What does `main()` do with the received messages?
   main() waits until enough messages have been received, stops the acceptor
   thread, checks whether the expected number of messages was received, then
   calls collect_all() to print and free all stored messages. It finally checks
   that all messages were collected.

4. How are threads used in this sample code?
   The program creates one acceptor thread that accepts incoming client
   connections. For each connected client, the acceptor creates a separate
   client thread. Each client thread reads messages from its socket and adds
   them to the shared linked list.


Explain the use of non-blocking sockets in this lab.
How are sockets made non-blocking?
   Sockets are made non-blocking by calling fcntl() to get the current file
   descriptor flags and then setting the O_NONBLOCK flag with F_SETFL.

What sockets are made non-blocking?
   The listening server socket is made non-blocking in run_acceptor(), and each
   connected client socket is made non-blocking in run_client().

Why are these sockets made non-blocking? What purpose does it serve?
   They are made non-blocking so the threads do not get stuck forever in
   accept() or read(). This lets the threads repeatedly check whether they
   should keep running, so the program can shut down cleanly while still
   handling clients and incoming messages.
*/
